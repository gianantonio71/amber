...
{
  return t.is_tagged_obj() && tag_type->contains(symbol_obj(t.get_tag())) && obj_type->contains(t.untag());
}
...
PtrnType::PtrnType(::pattern pattern) : pattern(pattern)
{
...
bool PtrnType::contains(Term &t)
{
  ...
  return pattern->matches(t, in_env, delta_env, true);
}
