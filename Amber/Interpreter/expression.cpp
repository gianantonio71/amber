...
static Expr mk_expr(ref<ExprObj> obj)
{
  Expr e;
  e.obj = obj;
  return e;
...
static ref<ExprObj> get_expr_obj(Expr e)
{
  return e.obj;
}
...
public:
  ...
  Term evaluate(Env &env, LocalDefs &ie)
  {
    ...
    return bool_obj(type->contains(res));
  }
...
public:
  ...
  Term evaluate(Term t, Env &env, LocalDefs &ie)
  {
    ...
    if (t.is_tagged_obj())
      return tagged_obj(t.get_tag(), evaluate(t.untag(), env, ie));
...
{
public:
	BasicSelectExpr(Expr src, type type) : SelectExprImpl(src), type(type)
	{
