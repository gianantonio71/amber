BoolProcDef mk_named_type_memb_test_fn(TypeSymbol type_name, (TypeSymbol => Type) typedefs)
{
  ...
  code := gen_type_checking_code(typedefs[type_name], fn_par(0), res_var)
          where
...
def
  ...
  [Instr+] gen_type_checking_code(Type type, AtomicExpr obj, BoolVar res_var):
    ...
    set_type() = {
      ...
      elem_var_id := next_obj_var_id;
      
      it_var   := set_it_var(it_var_id);
      elem_var := lvar(elem_var_id);
      ...
      code := [ maybe_op(block_success_if(is_empty_set(obj), res_var), not type.nonempty),
                ...
                repeat(
                  ...
                  gen_type_checking_code(type.elem_type, elem_var, res_var) &
    ...
    fixed_seq_type(ts) =
    {
      len_var_id  := next_int_var_id;
      it_var_id   := next_seq_it_var_id;
      elem_var_id := next_obj_var_id;
      
      len_var  := ivar(len_var_id);
      it_var   := seq_it_var(it_var_id);
      elem_var := lvar(elem_var_id);
    ...
    tuple_type(fs) =
    {
      size_var_id := next_int_var_id;
      obj_var_id  := next_obj_var_id;
      it_var_id   := next_map_it_var_id;
          
      size_var := ivar(size_var_id);
      obj_var  := lvar(obj_var_id);
      it_var   := map_it_var(it_var_id);
      ...
      sorted_fields := sort_set(fs)
                       where
                         is_strictly_ordered(f1, f2) = f1.label < f2.label;
                       ;

      for (b : sorted_fields)
        inner_code := [set_var(obj_var, get_curr_value(it_var))] &
                      gen_type_checking_code(b.type, obj_var, res_var) &
                      [exit_block_if_not(res_var), move_forward(it_var)]
                      where
                        next_int_var_id    = size_var_id + 1;
                        next_obj_var_id    = obj_var_id + 1;
                        next_map_it_var_id = it_var_id + 1;
                      ;
