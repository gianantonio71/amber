...
Instr call_cls(ObjVar v, ObjFnName n, [ObjExpr*] ps)  = call_cls(var: v, name: n, params: ps);

Instr cls_scope(ClsDef c, [Instr+] b) = cls_scope(cls: c, body: b);

//////////////////// //////////////////// ////////////////////

ObjProcDef obj_proc_def(ObjFnName name, Nat arity, [Instr+] body) =
  obj_proc_def(
    name:       name,
    in_arity:   arity,
    body:       body
...
Instr cls_scope(FnSymbol name, Int arity, [Var*] env, [Instr+] cls_body, [Instr+] body)
{
  cls := cls_def(
           name:   name,
           arity:  arity,
