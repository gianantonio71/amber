...
type SynObjErr    = TDefUserErr,
                    ...
                    dup_closure_def(BasicUntypedSng),
                    ...
                    //## NOT CHECKED YET
                    nested_local_fn(outer_fn: BasicUntypedSng, inner_fn: BasicUntypedSng);
...
UserErr* wf_errors(SynPrg prg)
{
  ...
  incomp_fn_sgns := for (s => fns <- fn_groups, fd1 <- fns, fd2 <- fns)
                      if (fd1 /= fd2, not syn_fns_are_compatible(fd1, fd2)) {
                        :incomp_fndefs(
                          name:   s.name,
                          arity:  s.arity,
                          params: {par_parts(fd1), par_parts(fd2)}
                        )
                      }
                    where
                      typedefs = inst_tdefs;
  ...
  undef_type_symb_errs := undef_type_symbol_errs(fndefs, ublocks) where typedefs = inst_tdefs;;
...
using
  typedefs			   	    -> (TypeSymbol => SynType);
  all_par_type_symbols 	-> [BasicTypeSymbol, NzNat]*;

def

  UserErr* tdef_errs(SynTypedef* tdefs) =
    for (td <- tdefs, es = type_wf_errors(td.type) where type_vars_in_scope = {};)
      if (es /= {}) {
        :tdef_err(
          type: td.name,
          errs: es
  ...
  UserErr* par_tdef_errs(SynParTypedef* par_tdefs) =
    for (td <- par_tdefs, es = type_wf_errors(td.type) where type_vars_in_scope = set(td.params);)
      if (es /= {}) {
        :tdef_err(
          type: td.name,
          errs: es
  ...
  UserErr* inst_tdef_errs((TypeSymbol => SynType) inst_tdefs) =
    for (s => t <- inst_tdefs, es = type_wf_errors(t) where type_vars_in_scope = select TypeVar in s end;)
      if (es /= {}) {
        :tdef_err(
          type: s,
          errs: es
  ...
  UserErr* fn_def_errs(SynFnDef* fndefs, UntypedSng* global_fns, BasicUntypedSng* impl_pars) =
  ...
  UserErr* ublock_errors(SynUsingBlock* ublocks, UntypedSng* global_fns) =
    union({block_wf_errors(b, global_fns) : b <- ublocks});

  UserErr* block_wf_errors(SynUsingBlock ublock, UntypedSng* global_fns)
  {
    //## UGLY UGLY
  ...
  TDefUserErr* sgn_wf_errors(Signature sgn)
  {
    type_vars := select TypeVar in sgn end; //## BAD BAD BAD THIS IS ALL WRONG.
    in_errs   := seq_union([type_wf_errors(t) : t <- sgn.params]) where type_vars_in_scope = type_vars;;
    out_errs  := type_wf_errors(sgn.res_type) where type_vars_in_scope = type_vars;;
