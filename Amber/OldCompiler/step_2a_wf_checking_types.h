...
Tautology types_are_wf((TypeSymbol => Type) typedefs)
{
  ...
  return not (? s => t <- typedefs : not type_is_wf(t, typedefs, select TypeVar in s end));
...
Tautology type_is_wf(Type type, (Any => Type) typedefs, TypeVar* type_vars):
  ...
  seq_type()          = type_is_wf(type.elem_type, typedefs, type_vars),
  
  fixed_seq_type(ts)  = all([type_is_wf(t, typedefs, type_vars) : t <- ts]),
  
  set_type()          = type_is_wf(type.elem_type, typedefs, type_vars),
  
  map_type()          = type_is_wf(type.key_type, typedefs, type_vars) and
                        type_is_wf(type.value_type, typedefs, type_vars),
  
  tuple_type(bs)      = { ls := apply(bs) where f(b) = b.label;;
                          return not (? l => c <- ls : c > 1) and
                                 not (? b <- bs : not type_is_wf(b.type, typedefs, type_vars));  
                        },
  
  //## BUG BUG BUG THIS IS INCOMPLETE, THE TAG TYPE MUST BE A SUBSET OF atom_type
  tag_type()          = type_is_wf(type.tag_type, typedefs, type_vars) and
                        type_is_wf(type.obj_type, typedefs, type_vars),
  ...
  union_type(ts)      = size(ts) >= 2                                             and
                        ...
                        not (? t <- ts : not type_is_wf(t, typedefs, type_vars))  and
                        //## BAD BAD BAD
