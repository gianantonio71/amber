...
def
  ...
  {
    fn_res_var_id := next_obj_var_id;
    fn_res_var := lvar(fn_res_var_id);
    code := gen_eval_code(expr, fn_res_var) where next_obj_var_id = fn_res_var_id + 1;;
  ...
  gen_eval_info(Expr expr)
  {
    ...
    var    := lvar(var_id);
    info   := gen_eval_info(expr, var) where next_obj_var_id = var_id + 1;; //## BUG? BUG? BUG?
  ...
  gen_eval_info([Expr*] exprs)
  {
    ...
    for (e : exprs)
      info := gen_eval_info(e) where next_obj_var_id = next_var_id;;
  ...
  {
    elems_var_id := next_vector_var_id;
    count_var_id := next_int_var_id;
    
    elems_var := vvar(elems_var_id, length(exprs));
    count_var := ivar(count_var_id);
    
    code := gen_vector_eval_code(exprs, elems_var, count_var)
            where
              next_vector_var_id = elems_var_id + 1;
              next_int_var_id    = count_var_id + 1;
            ;
    ...
    [Instr*] gen_vector_eval_code([SubExpr*] exprs, VecVar elems_var, IntVar count_var)
    {
      ...
      for (e : exprs)
        if (e :: Expr)
          ...
          cond_eval_info := gen_eval_info(e.cond, cond_var) where next_obj_var_id = cond_var_id + 1;;
  ...
  [Instr+] gen_eval_code(Expr expr, ObjVar res_var):
    ...
    seq_expr()      = { if (expr.tail?)
                        ...
                        if (expr.head /= [])
                          info := gen_vector_eval_info(expr.head) where next_obj_var_id = new_obj_var_id;;
                        ...
                        if (expr.tail?)
                          ...
                          info := gen_eval_info(expr.tail) where next_obj_var_id = new_obj_var_id;;
    ...
    map_expr(es)    = { return [set_var(res_var, empty_map)] if es = {};
    
                        cond_var_id   := next_obj_var_id;
                        keys_var_id   := next_vector_var_id;
                        values_var_id := next_vector_var_id + 1;
                        count_var_id  := next_int_var_id;
    
                        cond_var   := lvar(cond_var_id);
                        keys_var   := vvar(keys_var_id, size(es));
                        values_var := vvar(values_var_id, size(es));
                        count_var  := ivar(count_var_id);
                        ...
                        for (e : rand_sort(es))
                          key_code   := gen_eval_code(e.key, curr_key_slot_var)
                                        where
                                          next_vector_var_id = values_var_id + 1;
                                          next_int_var_id    = count_var_id + 1;
                                        ;
                          ...
                          if (e.cond?)
                            cond_info := gen_eval_info(e.cond, cond_var)
                                         where
    ...
    cls_call()      = { pars_info := gen_eval_info(expr.params);
                        call_code := [call_cls(res_var, expr.name, pars_info.exprs)];
                        return pars_info.eval_code & call_code & pars_info.cleanup_code;
                      },

                      //## BAD BAD BAD TOO SIMILAR TO THE ABOVE CODE
    builtin_call()  = { pars_info := gen_eval_info(expr.params);
    ...
    eq()            = { info := gen_eval_info([expr.left, expr.right]);
                        return info.eval_code &
                               [set_var(res_var, to_obj(is_eq(info.exprs[0], info.exprs[1])))] &
                               info.cleanup_code;
                      },

    membership()    = { bool_var_id := next_bool_var_id;
                        bool_var := bvar(bool_var_id);

                        info := gen_eval_info(expr.obj);
                        
                        code := gen_type_checking_code(expr.type, info.expr, bool_var) &
                                [set_var(res_var, to_obj(bool_var))]
                                where
                                  next_obj_var_id  = info.next_var_id;
                                  next_bool_var_id = bool_var_id + 1;
                                ;
                        
                        return info.eval_code & code & info.cleanup_code;
                      },

    accessor()      = { info := gen_eval_info(expr.expr);
    ...
    accessor_test() = { info := gen_eval_info(expr.expr);
    ...
    if_expr()       = { info := gen_eval_info(expr.cond);
    ...
    match_expr()    = { tmp_bvar := bvar(next_bool_var_id);
    
                        info := gen_eval_info(expr.exprs);

                        code := [terminate];
                        for (c : reverse(expr.cases))
                          case_code := gen_eval_code(c.expr, res_var) where next_obj_var_id = info.next_var_id;;
    ...
    set_comp() =
    {
      strm_var_id := next_stream_var_id;

      strm_var := svar(strm_var_id);

      action := eval_expr_and_add_to_set(expr.expr, strm_var);
      action := action(expr.sel_expr, action) if expr.sel_expr?;
      
      code := gen_iter_code(expr.source, action) where next_stream_var_id = strm_var_id + 1;;
    ...
    map_comp() =
    {
      key_strm_var_id   := next_stream_var_id;
      value_strm_var_id := next_stream_var_id + 1;
      
      key_strm_var   := svar(key_strm_var_id);
      value_strm_var := svar(value_strm_var_id);

      action := eval_exprs_and_add_to_map(expr.key_expr, expr.value_expr, key_strm_var, value_strm_var);
      action := action(expr.sel_expr, action) if expr.sel_expr?;
      
      code := gen_iter_code(expr.source, action) where next_stream_var_id = value_strm_var_id + 1;;
    ...
    seq_comp() =
    {
      ...
      item_var_id := next_obj_var_id + 1;
      sel_var_id  := next_obj_var_id + 2;
      it_var_id   := next_seq_it_var_id;
      strm_var_id := next_stream_var_id;
      idx_var_id  := next_int_var_id;
      
      src_var  := lvar(src_var_id);
      item_var := lvar(item_var_id);
      it_var   := seq_it_var(it_var_id);
      sel_var  := lvar(sel_var_id);
      strm_var := svar(strm_var_id);
      idx_var  := ivar(idx_var_id); //## BAD BAD BAD USED EVEN WHEN NOT REQUIRED

      //## BAD BAD BAD THE WHERE BLOCK IS REPEATED THREE TIMES
      src_info := gen_eval_info(expr.src_expr, src_var)
                  where
                    next_obj_var_id    = sel_var_id  + 1;
                    next_seq_it_var_id = it_var_id   + 1;
                    next_stream_var_id = strm_var_id + 1;
                    next_int_var_id    = idx_var_id  + 1;
                  ;
      ...
      sel_info := nil;
      if (expr.sel_expr?)
        sel_info := gen_eval_info(expr.sel_expr, sel_var)
                    where
                      next_obj_var_id    = sel_var_id  + 1;
                      next_seq_it_var_id = it_var_id   + 1;
                      next_stream_var_id = strm_var_id + 1;
  ...
  [Instr+] gen_code(Statement stmt, ObjVar res_var, ObjVar* all_rel_vars, ObjVar* break_vars):
    ...
    assignment_stmt() =
    {
      ...
      if (in(stmt.var, all_rel_vars))
        if (in(stmt.var, extern_vars(stmt.value)))
          ...
          code := gen_eval_code(stmt.value, tmp_var) where next_obj_var_id = tmp_var_id + 1; &
    ...
    {
      tmp_var_id   := next_obj_var_id;
      start_var_id := next_int_var_id;
      end_var_id   := next_int_var_id + 1;
      idx_var_id   := next_int_var_id + 2;
      
      tmp_var   := lvar(tmp_var_id);
      start_var := ivar(start_var_id);
      end_var   := ivar(end_var_id);
      idx_var   := ivar(idx_var_id);
      
      start_eval_code := gen_eval_code(stmt.start_val, tmp_var) where next_obj_var_id = tmp_var_id + 1;;
      end_eval_code   := gen_eval_code(stmt.end_val, tmp_var) where next_obj_var_id = tmp_var_id + 1;;
      
      body_code := gen_code(stmt.body, res_var, all_rel_vars, {}, {})
                     where
    ...
    foreach_stmt() =
    {
      ...
      src_info := gen_eval_info(stmt.values, src_var) where next_obj_var_id = src_var_id + 1;;
      
      body_code := gen_code(stmt.body, res_var, all_rel_vars & {src_var if src_info.var_used}, {}, {})
                   where
                     next_obj_var_id    = src_var_id + if src_info.var_used then 1 else 0 end;
                     next_int_var_id    = idx_var_id + if stmt.idx_var? then 1 else 0 end;
                     next_seq_it_var_id = it_var_id + 1;
                   ;
...
Expr simplify(Expr expr)
{
  ...
  Expr make(Atom fn_name, Expr src_expr, Pattern ptrn, Expr sel_expr, Expr expr)  = 
    where_expr(
      expr:   fn_call(name: :fn_symbol(fn_name), params: [src_expr]),
      fndefs: {
