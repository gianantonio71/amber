...
def
  ...
  SynObjErr* fndef_wf_errors(SynFnDef fn_def, UntypedSng* global_fns, BasicUntypedSng* impl_pars) =
  ...
  SynObjErr* fndef_wf_errors(SynFnDef fn_def, UntypedSng* global_fns, TypeVar* type_vars, BasicUntypedSng* impl_pars)
  {
    ...
    for (p : fn_def.params)
      if (p.type?)
        sgn_errs := sgn_errs & type_wf_errors(p.type) where type_vars_in_scope = ts;;
      ;
    ...
    ret_type_errs := if fn_def.res_type?
                       then type_wf_errors(fn_def.res_type) where type_vars_in_scope = ts;
                       else {}
                     end;

    loc_fns_errs  := seq_union([fndef_wf_errors(fn, all_fns, ts, impl_pars) : fn <- fn_def.local_fns]);
    
    expr_errs     := expr_wf_errors(fn_def.expr, vs)
                     where
...
def
  ...
  SynObjErr* expr_wf_errors(SynExpr expr, Var* def_vars):
    ...
    const_or_var(a)     = { return {} if in(:var(a), def_vars) or is_def(:fn_symbol(a), 0, fns_in_scope);
                            return if is_almost_def(:fn_symbol(a), 0, fns_in_scope)
                                     then {:almost_def_const(a)}
                                     else {:undef_var_or_const(a)};;
                          },

    fn_call()           = { errs := exprs_wf_errors(expr.params, def_vars);
                             if (not is_def(expr.name, length(expr.params), fns_in_scope))
    ...
    where_expr()        = { ips := impl_params & {untyped_sgn(fd) : fd <- set(expr.fndefs)};
                            expr_errs  := expr_wf_errors(expr.expr, def_vars) where impl_params = ips;;
