...
using
  closures      -> BasicUntypedSng*;
  local_fns     -> BasicUntypedSng*;
  curr_outer_fn -> FnSymbol;
...
def
  ...
  Expr desugar_expr(SynExpr expr, Var* def_vars):
    ...
    const_or_var(a) = { return :var(a) if in(:var(a), def_vars);
                        fn  := :fn_symbol(a);
                        sgn := untyped_sgn(name: fn, arity: 0);
                        ...
                        if (in(sgn, closures))
                          ...
                          return fn_call(name: fn, params: []);
                        ;
                      },

    fn_call()       = { ps  := [desugar_expr(e, def_vars) : e <- expr.params];
                        ...
                        if (in(sgn, local_fns))
                          ...
                          return fn_call(name: fs, params: ps);
                        ;
                         
                        // Then closures
                        return cls_call(name: expr.name, params: ps) if in(sgn, closures);
                        
                        // And last global functions
                        return fn_call(name: expr.name, params: ps);
                      },
...
//               match_expr(exprs: [SynExpr+], cases: [SynCase+]),
  ...
  Statement desugar_stmt(SynStmt stmt, Var* def_vars):
    ...
    if_stmt()         = { res := desugar_stmts(stmt.else, def_vars);
    ...
    loop_stmt()       = { cond      := desugar_expr(stmt.cond, def_vars);
    ...
    for_stmt()        = { //## ALSO MAKE SURE SYNTAX CHECK GETS IT RIGHT
                          iters  := stmt.loops;
                          vs     := def_vars;
                          for_vs := [];
                          for (it : iters)
