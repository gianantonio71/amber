...
Term mk_seq(Term first_element)
{
  if (!first_element.is_null())
    return seq_obj(mk_v(first_element));
  else
    return null();
}
...
Term mk_seq(Term seq, Term tail_element)
{
  if (!seq.is_null() && !tail_element.is_null())
    return seq_obj(mk_v(seq.items(), tail_element));
  else
    return null();
}
...
{
  return seq_obj(mk_v(element, sequence.items()));
}
...
Term mk_fn_name(Term name)
{
  ...
  if (is_valid_identifier(s))
    return tagged_obj("fn_symbol", symbol_obj(s));
  
  assert(is_valid_operator_as_function(s));
  
  return tagged_obj("op_symbol", symbol_obj(translate_op(s.substr(3))));
}
...
{
  return tagged_obj("var", make_atom(token));
}
...
{
  return tagged_obj("type_symbol", make_atom(token));
}
...
Term mk_fndef_switch(Term ret_type, Term fn_name, Term fn_args, Term branches, Term local_fns)
{
  ...
  for (int i=0 ; i < par_count ; i++)
    mes[i] = tagged_obj("fn_par", int_obj(i));
...
{
  return tagged_obj("obj_ptrn", make_symb_obj(symb));
}

Term mk_ptrn_num(Term num)
{
  return tagged_obj("obj_ptrn", make_int_obj(num));
}
...
{
  return tagged_obj("ext_var_ptrn", make_var(var));
}
...
{
  return tagged_obj("type_ref", make_basic_type_symbol(name));
}

Term mk_type_var(Term var_name)
{
  return tagged_obj("type_var", make_atom(var_name));
}

Term mk_type_ref(Term name, Term type_pars)
{
  return tagged_obj("type_ref", make_par_type_symbol(name, type_pars));
}
...
{
  return tagged_map("seq_type", "elem_type", type, "nonempty", bool_obj(nonempty));
}
...
{
  return tagged_map("set_type", "elem_type", elem_type, "nonempty", bool_obj(nonempty));
}
...
{
  return tagged_obj("symb_type", make_symb_obj(symb));
}
...
{
  return tagged_obj("set_expr", seq_to_set(subexprs));
}
...
{
  return tagged_obj("map_expr", seq_to_set(entries));
}

Term mk_expr_tuple(Term labexprs)
{
  return tagged_obj("map_expr", seq_to_set(labexprs));
}
...
{
  return tagged_obj("const_or_var", make_atom(const_or_var_name));
}
...
{
  return tagged_map("fn_call",
           "name",         tagged_obj("op_symbol", symbol_obj(translate_op(op.get_string()))),
           "params",       mk_seq(val),
           "named_params", mk_seq_empty()
         );
}
...
Term make_expr_idx(Term obj, Term params)
{
  ...
  return tagged_map("fn_call",
           "name",         tagged_obj("op_symbol", symbol_obj("brackets")),
           "params",       seq_obj(ps),
           "named_params", mk_seq_empty()
         );
}
